...
def precipitation():
    ...
    pastyear = session.query(func.date(func.max(measurement.date), '-1 year')).scalar()

    # Perform a query to retrieve the data and precipitation scores

    results = session.query(measurement.date, measurement.prcp).filter(measurement.date >= pastyear).all()
